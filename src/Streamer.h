...
namespace minirisk {
	...
	{
		union { double dfloat; uint64_t int64; } tmp;
		tmp.dfloat = v;
		os.m_of << std::hex << tmp.int64 << separator;
		return os;
	}

	// when reading a double from a file in text format, convert int64 to double
	inline double& operator>>(my_ifstream& is, double & db)
	{
		union { double dfloat; uint64_t int64; } tmp;
		string temp;
		char* str;
		is >> temp;
		auto c = std::strtoll(temp.c_str(), &str, 16);
		tmp.int64 = c;
		db = tmp.dfloat;
		return tmp.dfloat;
	}
	...
	inline my_ifstream& operator>>(my_ifstream& is, Date& v)
	{
		...
		{
			v.init_reversal(std::atoi(tmp.c_str())); // initialize serial type
		}
		return is;
	}
