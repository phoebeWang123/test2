...
\abstract{
	This source code constitutes the foundation of a risk management system. It can load trades from a database, construct market objects as needed, compute the price of trades and their sensitivities with respect to risk factors.

A real risk management system entails of many more features: sophisticate market objects and pricing models; efficient algorithms to avoid unnecessary recomputations when computing Greeks; support for a large number of trade types; an extensive set of tools to orchestrate recomputations and postprocess their results (e.g. P\&L explain, VaR, PFE, XVA); tools to life-cycle trades; and support for parallel computations (thread safety).
