...
\section{What to do}
You have to complete sequentially all tasks mentioned in the section \ref{sec:task}. After completing each task you should be able to succesfully run the command\\
...
style
comments
minimal changes (git diff)

\section{Tasks}
\label{sec:task}

\subsection{Improving the \textit{Date} class}
...
Add to the 2 functions constructed in section \ref{sec:}

\subsection{Minimize market objects recalculations}

\subsection{Implement query which returns risk factor for a trade}

\subsection{Minimize trade price recomputations}
